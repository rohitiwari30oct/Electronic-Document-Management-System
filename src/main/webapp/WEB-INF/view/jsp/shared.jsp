...
<input type="hidden" id="breadcumPath" value="<%=(String)request.getAttribute("breadcumPath") %>" />
...
<div class="strip_details index_heading">
 ...
 %>  <span id='/<%=userid%>' style='cursor: pointer'
			onclick='getSharedFileSystem(this.id)'>Shared With Me / </span> <%
  	String bdm="";
  	...
  	for(int i=1;i<breadcm.length;i++){
  		...
  		if(i==1){
  			 %>
  			<%--  <span id='<%="/"+userid%>' style='cursor: pointer'
  						onclick='getSharedFileSystem(this.id)'> --%> <%-- </span> 
  						<script type="text/javascript">
 ...
 %>
 <span id='<%=bdm%>' style='cursor: pointer'
			onclick='getSharedFileSystem(this.id)'><%=breadcm[i]%> /
			<%} %>
		...
		<div class="list_view_middle">
			...
			<div class="list_view_content">
				...
				<ul  
					...
					ondblclick="getSharedFileSystem(this.id)"
						oncontextmenu="getDocProperties(this.id);getRightClickMenuFolder(this.id)" >
					<li>
...
<!----------/// RIGHT CLICK MENU STRED HERE ---------->
		...
		<div class="right_click_menu_blank">
			...
			<%} %>
				...
				<li class="rename_right" id="refresh<%=currentFolder.getFolderPath() %>"   onclick="getSharedFileSystem(this.id.substring(7))" ><div class="right_images">
						<img src="images/paste_icon.png" />
						<div class="clear"></div>
					</div>
					<span>refresh</span>
		...
		<div class="right_click_menu_file">
			<ul>
				...
				<li id="fileRightMenuSharing" class="share_right mange_sharing"><div class="right_images">
					...
					<span>share</span>
				<div class="clear"></div></li>
				<li id="fileRightMenuGetLink" class="get_link"><div class="right_images">
						<img src="images/share_linl.png" />
						<div class="clear"></div>
					</div>
					<span>get link</span>
