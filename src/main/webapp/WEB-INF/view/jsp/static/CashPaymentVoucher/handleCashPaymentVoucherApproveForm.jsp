...
function valiForm(id,dec)
{
	...
	if(pg==true)
		...
		try{
			...
			today = dd+'-'+mm+'-'+yyyy;
			if(dec=='accept'){
				$('#historyHiddenTasks').attr("value",$('#historyHiddenTasks').val()+"Approved By " + $("#formBy").val()+" on Dated "+today+",") ;
            ...
            <input type="button" onclick="getHistoryForms()" class="getHistory"  value="Get History" />
                 ...
                 <td colspan="4"><div class="table_right">
								...
								<input type="button" id="CPVAppr"
									...
									value="APPROVE" onclick="valiForm(this.id,'accept')">
								<!----------/// ADD USER END -------->
								<!----------/// User List ----------->
								<!--   <div class="Logged_User_tab  onclick">CANCEL</div> -->
								<input type="button" class="Logged_User_tab  onclick"
									value="REJECT"  id="myrej" 
									 onclick="valiForm(this.id,'reject')">
								<!----------/// User List END -------->
<input type="button"
								class="Logged_User_tab_color add_user_js add_user_bg"
								value="Print"
