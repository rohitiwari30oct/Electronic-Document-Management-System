...
// 			 alert("asdf");
		if (pg == true) {
			...
			travelExpensesReimbursement_submit('travelReimbursementRequest::Travel Reimbursement');
		}}
	}
		...
		<div class="from_main_leave left_space_one">
			<table  id="beginWorkflowForm">
				<tbody>
					<tr>
						...
						<td colspan="5" class="table_heading">TRAVEL EXPENSES
							REIMBURSEMENT CLAIM<input type="hidden" value="<%=dt %>" name="filledDaete" id="filledDaete" />
                <input type="hidden" value="Initiated" name="formSStatus"  id="formSStatus" />
							...
							<td>DATE
							<div class="claer"></div> <input value="<%=dt %>" type="text" onblur="myVal(this.id)"  class="popupDatepicker"  id="requestDate">
						</td>
					</tr>
					<tr class="depart_form">
						<td colspan="2">NAME
							<div class="claer"></div> <input type="text" onblur="myVal(this.id)" value="<%=uname %>"   id="empName" class="blank_val">
						</td>
						<td colspan="2">EMP.NO
							<div class="claer"></div> <input type="text" onblur="myVal(this.id)"  id="empNo" value="<%=eno %>" />
						</td>
...
1. All Claims must have supporting bill except *.
               ...
               <option value="index">Please Select </option>
<!--                <option value="demo2@avi-oil.com">demo2</option> -->
					...
					<tr>
						<td colspan="7"><div class="table_right">
								<input type="button" id="purplantsub"
									...
									value="Submit" onclick="checkMail(this.id)">
								<input type="button"
									class="Logged_User_tab_color add_user_js add_user_bg"
									value="Print" onclick="printDivPurPlant()">
							</div></td>
					</tr>
